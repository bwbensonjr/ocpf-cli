...
- [x] 1.1 Extend `districts._normalize` to fold ordinal words to digits
...
- [x] 1.2 Confirm `uv run ocpf race "First Suffolk" --year 2026` resolves, and
...
- [x] 2.1 Change `resolve_district(query, districts=None)` to require a `year`
...
- [x] 2.2 Keep tier 1 first and short-circuiting: when the current `districts`
...
- [x] 3.1 Add a parser turning a depository-feed `officeSought` string such as
...
- [x] 3.2 Resolve a name against the feed's `officeSought` values for the
      ...
      call is made.
- [x] 3.3 Record why the tier stops at 2020: verify against the live API that
...
- [x] 4.1 Implement the `onballot/finsummaries/{year}/{code}` sweep over an
...
- [x] 4.2 Label each populated code with its era-correct district via
...
- [x] 4.3 Sweep Senate before House and short-circuit on a hit; verify with a
      mocked API that a Senate match issues no House probes (design Decision 2).
- [x] 4.4 Emit progress to stderr during a sweep so a slow resolution is visibly
      working; verify stdout stays clean so `--json` output remains pipeable.
- [x] 4.5 Verify end to end against the live API that
...
- [x] 5.1 Distinguish "not a district name", "known but not in this year" (naming
      ...
      design Decision 4).
- [x] 5.2 Verify `uv run ocpf race "Worcester and Norfolk" --year 2026` reports
      that the district did not exist in 2026 rather than that the name is not a
      legislative district.

## 6. Documentation

- [x] 6.1 Record in `CLAUDE.md`: `districts` is the current map only and omits
      ...
      cited.
- [x] 6.2 Note the year-aware behavior in `README.md` under `ocpf race`,
...
- [x] 6.3 Add a CHANGELOG entry under an unreleased heading describing the fixed
      lookup and the ordinal folding.
- [x] 6.4 Run `uv run pytest` and confirm the suite passes, with existing race
